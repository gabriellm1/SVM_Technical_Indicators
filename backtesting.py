    ...
    def loadYAHOOHist(self, file, instrument, type=Event.CANDLE):
        ...
        for event in events:
            ...
            if len(cols) == 7 and cols[1] != 'null':
                ...
                quantity = int(float(cols[6])) # mexi aqui tava 0
...
class Strategy():
    ...
    def fill(self, id, instrument, price, quantity, status):
        ...
        if price != 0:
            ...
            if quantity > 0 and self._position[instrument] > 0:
                self._notional[instrument] += quantity*price
            elif quantity < 0 and self._position[instrument] < 0:
                self._notional[instrument] -= quantity*price
    ...
    def summary(self, tax=0.000, fee=0):
        ...
        if nt > 0:
            ...
            avg = self._legs[0][0]/(self._legs[0][1])

            i = 1
            while i < len(self._legs):
                res = self._legs[i][0]-self._legs[i-1][0]
                amo = self._legs[-1][1] # (self._legs[i][1]-self._legs[i-1][1])/2
                avg += res/amo
